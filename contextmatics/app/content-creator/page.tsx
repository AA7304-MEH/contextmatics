                        ...
                        <div className="mb-10 text-left">
                            ...
                            {inputContent.length > 0 && (
                                <div className="flex justify-end mt-2">
                                    <button onClick={() => setInputContent('')} className="text-xs text-red-400 hover:text-red-300 transition-colors">
                                        Clear Input
                                    </button>
                                </div>
                            )}
                        ...
                        <div className="card p-8 bg-background-surface/50 border border-white/5 flex flex-col h-full animate-fade-in-up">
                            <div className="flex justify-between items-center mb-6 text-left">
                                ...
                                <div className="flex gap-2">
                                    <button onClick={() => setShowComparison(!showComparison)} className={`btn py-1.5 px-3 text-xs transition-all ${showComparison ? 'bg-blue-500/20 text-blue-400 border-blue-500/30' : 'btn-secondary'}`}>
                                        {showComparison ? '✕ Close Compare' : '⇆ Compare'}
                                    </button>
                                    <button onClick={copyToClipboard} className="btn btn-secondary py-1.5 px-3 text-xs">
                                        Copy
                                    </button>
                                </div>
                            </div>
                            ...
                            {showComparison ? (
                                <div className="grid grid-cols-2 gap-4 flex-1 text-left">
                                    <div>
                                        ...
                                        <textarea readOnly value={inputContent} className="input w-full min-h-[350px] bg-black/30 resize-none leading-relaxed font-mono text-sm p-4 rounded-xl border border-white/10 text-text-secondary" />
                                    </div>
                                    <div>
                                        <p className="text-xs text-blue-400 uppercase tracking-wider mb-2 font-medium">Generated Output</p>
                                        <textarea readOnly value={generatedResult} className="input w-full min-h-[350px] bg-black/30 resize-none leading-relaxed font-mono text-sm p-4 rounded-xl border border-blue-500/20 text-text-primary" />
                                    </div>
                                </div>
                            ) : (
                                <div className="relative flex-1 text-left">
                                    <textarea readOnly value={generatedResult} className="input w-full h-full min-h-[400px] bg-black/30 resize-none leading-relaxed font-mono text-sm p-6 rounded-xl border-dashed border-2 border-white/10 focus:border-brand-primary/30 transition-colors text-text-primary" />
                                </div>
                            )}

                            <div className="flex gap-4 mt-6 pt-6 border-t border-white/5">
                                <button onClick={() => router.push('/history')} className="btn btn-secondary flex-1">
                                    View History
                                </button>
                                <button onClick={handleGenerate} className="btn btn-primary flex-1 bg-white/5 hover:bg-white/10 border-white/10">
                                    Regenerate
                                </button>
                            </div>
                        </div>
